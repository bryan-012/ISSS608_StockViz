...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      textInput("stock_symbol", "Stock Symbol (e.g., AAPL)", value = ""),
      numericInput("quantity", "Number of Shares", value = 10, min = 1),
      dateInput("buy_date", "Buy Date", value = Sys.Date() - 30),
      textInput("buy_price", "Buy Price (optional)", value = ""),
      actionButton("add_stock", "+ Add to Portfolio"),
      actionButton("remove_selected", "❌ Remove Selected Entries"),
      br(),
...
server <- function(input, output, session) {
  ...
  output$rolling_vol_plot <- renderPlot({
    ...
    ggplot(df %>% filter(!is.na(rolling_vol)), aes(x=date, y=rolling_vol, color=symbol)) +
      geom_line() +
      labs(title="20-Day Rolling Volatility (Standard Deviation of Daily Returns)", y="Volatility", x="Date") +
