...
server <- function(input, output, session) {
  ...
  output$portfolio_table <- renderDT({
    ...
    if (nrow(df) > 0) {
      ...
      df$buy_price <- mapply(function(sym, date, price) {
        if (is.na(price)) {
          # If user didn't input, retrieve the median market price from stock_data
          if (!is.null(rv$stock_data)) {
            ...
            return(paste0("$", round(market_price, 2), " (Used Market Price)"))
          } else {
            return("NA (Market Price)")
          }
        } else {
  ...
  output$monthly_volume_plot <- renderPlot({
    ...
    ggplot(df, aes(x=month, y=total_volume, fill=factor(year))) +
      ...
      labs(title="📊 Monthly Trading Volume by Stock (Past 5 Years)", x="Month", y="Volume", fill="Year") +
